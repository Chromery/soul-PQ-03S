...
for (const groupScope of [false, true]) test(`presentation grouping in ${groupScope ? "study group" : "study"}: defaults, edits, dissolve, persistence and partial export`, async ({ page }) => {
  ...
  expect(overrides["group:valuation:original-group:categoria"]).toBe("D/4");
  expect(overrides["PG-1:categoria"]).toBeUndefined();
  expect(overrides["PG-2:categoria"]).toBeUndefined();
  const groupAddress = groupRow.getByRole("textbox", { name: /^indirizzo del/ });
  await groupAddress.fill("Complesso Via Roma 44"); await groupAddress.press("Enter"); await saved();
  expect(overrides["group:valuation:original-group:indirizzo"]).toBe("Complesso Via Roma 44");
  expect(overrides["PG-1:indirizzo"]).toBeUndefined();
  expect(overrides["PG-2:indirizzo"]).toBeUndefined();
  ...
  // Reproduce the old workaround: blank descriptive member fields must not block a valid group.
  overrides["PG-1:indirizzo"] = ""; overrides["PG-2:categoria"] = "";
  await page.reload(); await saved();
  await page.getByRole("button", { name: "Generazione Presentazione", exact: true }).click();
  await modal.getByRole("button", { name: /Espandi gruppo/ }).click();
  ...
  await expect(modal.getByRole("button", { name: "Genera PDF v3", exact: true })).toBeEnabled();
  await modal.getByRole("button", { name: "Genera PDF v3", exact: true }).click();
  await expect(modal.getByText("PDF v3 pronto", { exact: true })).toBeVisible();
  ...
  expect(properties[0].currentRendita).toBe(100.10);
  await modal.getByRole("button", { name: "Chiudi", exact: true }).last().click();
  await groupRow.getByRole("button", { name: /^Ripristina categoria del/ }).click(); await saved();
  await expect(groupCategory).toHaveValue("D/7");
  expect(overrides["PG-2:categoria"]).toBe("");
  overrides["PG-1:categoria"] = "D8"; overrides["PG-2:categoria"] = "D/8 ";
  await page.reload(); await saved();
  await expect(groupCategory).toHaveValue("D/8");
