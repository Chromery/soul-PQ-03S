...
for (const groupScope of [false, true]) test(`presentation grouping in ${groupScope ? "study group" : "study"}: defaults, edits, dissolve, persistence and partial export`, async ({ page }) => {
  ...
  await expect(preview.locator(".presentation-group-row")).toContainText("Sub. 1, 2");
  await expect(preview.locator(".presentation-group-row")).toContainText("300,30");
  ...
  await expect(preview.locator(".presentation-group-row")).toContainText("310,30");
  ...
  await saved(); await expect(preview.locator(".presentation-group-row")).toContainText("Sub. 1, 2");
  await expect(preview.locator(".presentation-group-row")).toContainText("310,30");
